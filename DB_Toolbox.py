...
class SQL_Database:
    ...
    def dbGetTableList(self): 
        ...
        TableList = []
        for Table in self.dbExecute(r"SELECT * FROM sqlite_master WHERE type='table';"):
    ...
    def dbGetTableColumnDefinition(self, Table_Name):
        ...
        Regex_Results = Regex_Results.replace('\n', ' ').strip().split(', ')
    ...
    def dbRecordDelete(self, Table_Name, Record_Definition):
        ...
        SQL_Command = f"""DELETE FROM {Table_Name} WHERE {SQL_Command_sub}"""
    ...
    def dbRecordInsert(self, Table_Name, Record_Definition, Replace_Existing = False):
        ...
        SQL_Command = f"""INSERT{SQL_Command_Sub3} INTO {Table_Name} 
            ({SQL_Command_Sub1}) 
...
db = SQL_Database(r'C:\Users\lkapp\Desktop\IKOCatalogue_release.db')
